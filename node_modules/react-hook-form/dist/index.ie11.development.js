...
function useForm(_a) {
    ...
    function clearErrors(name) {
        name &&
            (isArray(name) ? name : [name]).forEach(function (inputName) {
                return unset(formState.errors, inputName);
            });
        updateFormState({
    ...
    function setError(name, error) {
        set(formState.errors, name, __assign(__assign({}, error), { ref: (fieldsRef.current[name] || {}).ref }));
        updateFormState({
            isValid: false,
            errors: formState.errors,
        });
            ...
            switch (_e.label) {
                ...
                case 10:
                    if (!(isEmptyObject(fieldErrors) &&
                        Object.keys(formState.errors).every(function (name) {
                ...
                case 11:
                    ...
                    return [3 /*break*/, 15];
                case 12:
                    formState.errors = __assign(__assign({}, formState.errors), fieldErrors);
...
var useFieldArray = function (_a) {
    ...
    var _c = control || methods.control, isWatchAllRef = _c.isWatchAllRef, resetFieldArrayFunctionRef = _c.resetFieldArrayFunctionRef, fieldArrayNamesRef = _c.fieldArrayNamesRef, fieldsRef = _c.fieldsRef, defaultValuesRef = _c.defaultValuesRef, removeFieldEventListener = _c.removeFieldEventListener, _d = _c.formStateRef.current, dirtyFields = _d.dirtyFields, touched = _d.touched, errors = _d.errors, updateFormState = _c.updateFormState, readFormStateRef = _c.readFormStateRef, watchFieldsRef = _c.watchFieldsRef, validFieldsRef = _c.validFieldsRef, fieldsWithValidationRef = _c.fieldsWithValidationRef, fieldArrayDefaultValues = _c.fieldArrayDefaultValues, validateResolver = _c.validateResolver, renderWatchedInputs = _c.renderWatchedInputs, getValues = _c.getValues;
    var rootParentName = getFieldArrayParentName(name);
    var getDefaultValues = function () { return __spread((get(fieldArrayDefaultValues.current, rootParentName)
    ...
    var append = function (value, shouldFocus) {
        ...
        if (readFormStateRef.current.dirtyFields ||
            readFormStateRef.current.isDirty) {
            set(dirtyFields, name, __spread(get(dirtyFields, name, fillEmptyArray(fields)), filterBooleanArray(value)));
            updateFormState({
                isDirty: true,
